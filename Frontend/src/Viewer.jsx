...
export default function Viewer() {
  ...
  const performUndo = useCallback(() => {
    ...
    if (lastState.type === "model") {
      ...
      if (modelData && modelData.scene) {
        ...
        setUndoNotification({
          type: "success",
          message: "Model transform undone",
        });
      }
      ...
      if (modelData && modelData.scene) {
        ...
        setUndoNotification({
          type: "success",
          message: "Material mesh transform undone",
        });
      }
    }

    setUndoHistory((prev) => prev.slice(0, -1));
    // Clear any material selection after undo so UI/state doesn't keep a stale selection
  ...
  // File Processing
  const processFile = useCallback((file, placementIndex = null) => {
    setError("");
    setWarning("");
    setLoading(true);
    ...
    loadModel(file, (status) => setLoadingStatus(status))
      .then((scene) => {
        ...
        // Determine placement to avoid overlapping models when uploading multiple files
        const baseIndex =
          placementIndex !== null ? placementIndex : modelsRef.current.length;

        const bbox = new THREE.Box3().setFromObject(scene);
        const size = bbox.getSize(new THREE.Vector3());
        const maxDim = Math.max(size.x, size.y, size.z, 1);
        const spacing = maxDim * 1.3; // provide small gap between models
        const xOffset = baseIndex * spacing;

        try {
          scene.position.set(xOffset, 0, 0);
          scene.updateMatrixWorld(true);
        } catch (err) {
          // ignore if scene isn't mutable
        }
  ...
  const processMultipleFiles = useCallback(
    (files) => {
      ...
      const baseIndex = modelsRef.current.length;
      fileArray.forEach((file, i) => {
        const placementIndex = baseIndex + i;
        setTimeout(() => processFile(file, placementIndex), i * 100);
      });
    },
    [processFile]
  );
  ...
  const handleToggleModelVisibility = useCallback((modelId) => {
    setModels((prev) =>
      prev.map((m) => (m.id === modelId ? { ...m, visible: !m.visible } : m))
    );
  }, []);
  ...
  const handleSelectMaterial = useCallback((materialUuid, modelId = null) => {
    ...
    const targetModels = modelId
      ...
      ? currentModels.filter((m) => m.id === currentSelectedModelId)
      : currentModels;
  ...
  const handleMaterialMeshesUpdate = useCallback(
    (meshes) => {
      const currentSelectedModelId = selectedModelIdRef.current;
  ...
  // =========================================================
  const handleFocusOnMaterial = useCallback(
    (materialUuid) => {
      const currentModels = modelsRef.current;
      ...
      const newPos = new THREE.Vector3()
        .copy(center)
        .add(direction.multiplyScalar(fitOffset));
      ...
      <div className="flex-1 relative overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Upload STEP, OBJ, FBX, or GLTF files
              </p>
              <p className="text-xs text-gray-600">
                Drag & drop multiple files to load them together
              </p>
            </div>
          </div>
        )}
        ...
        >
          ...
          <BackgroundClickDetector
            ...
            enabled={
              selectedMaterialId !== null && materialTransformMode === "none"
            }
          />
              ...
              (modelData) =>
                modelData.visible && (
                  <Model
                    ...
                    transformMode={
                      selectedModelId === modelData.id ? transformMode : "none"
                    }
                    onTransformChange={setIsTransforming}
                    onModelUpdate={(updated) =>
                      handleModelUpdate(modelData.id, updated)
                    }
                    onSelect={() => handleSelectModel(modelData.id)}
                    highlightedMeshes={
                      selectedModelId === modelData.id
                        ? highlightedMeshes.filter(
                            (h) => h.modelId === modelData.id
                          )
                        : []
                    }
                    selectedMaterialId={
                    ...
                    enableMaterialPicking={
                      isMaterialPickingEnabled &&
                      selectedModelId === modelData.id
                    }
                  />
                )
        ...
        {undoNotification && (
          <div
            className={`absolute top-16 left-1/2 -translate-x-1/2 px-4 py-2 rounded-lg backdrop-blur-sm z-50 transition-all animate-fadeIn ${
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-white font-medium">
                    {formatNumber(totalStats.vertices)}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-white font-medium">
                    {formatNumber(totalStats.triangles)}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-purple-300">
                    {highlightedMeshes.length} mesh
                    {highlightedMeshes.length !== 1 ? "es" : ""} selected
                  </span>
                </div>
                <button
